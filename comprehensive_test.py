"""
Script de test complet pour l'application de gestion de stock
...
class TestDatabaseFunctionality(unittest.TestCase):
    ...
    def test_stock_movements(self):
        ...
        self.assertEqual(last_movement[3], "RETRAIT")
        self.assertEqual(last_movement[4], 3)
        self.assertEqual(last_movement[5], "PROJET2")
        self.assertEqual(last_movement[6], "USER2")
    ...
    def test_low_stock_detection(self):
        ...
        self.assertEqual(len(low_stock), 2)  # REF001 et REF002
...
class TestStockManagerFunctionality(unittest.TestCase):
    ...
    def test_excel_import_export(self):
        ...
        # Exporter les articles à stock bas
        result, message = self.stock_manager.export_low_stock_to_excel(export_file, "Swisspro24")
        ...
        result, message, stats = self.stock_manager.import_from_excel(import_file, "Swisspro24")
        self.assertTrue(result)
        
        # Vérifier les statistiques d'import
        self.assertEqual(stats['added'], 2)  # 2 nouveaux articles
        self.assertEqual(stats['updated'], 1)  # 1 article mis à jour
        ...
        # Vérifier l'article mis à jour
        article = self.stock_manager.get_article_by_reference("REF001")
        self.assertEqual(article['quantite'], 20)  # 12 + 8
    
    def test_password_protection(self):
        """Test de la protection par mot de passe"""
        export_file = os.path.join(self.temp_dir, "export_test.xlsx")
        
        # Tenter d'exporter avec un mot de passe incorrect
        result, message = self.stock_manager.export_low_stock_to_excel(export_file, "MotDePasseIncorrect")
        self.assertFalse(result)
        self.assertEqual(message, "Mot de passe incorrect")
        
        # Tenter d'exporter avec le bon mot de passe
        result, message = self.stock_manager.export_low_stock_to_excel(export_file, "Swisspro24")
        self.assertTrue(result)

class TestExternalDBFunctionality(unittest.TestCase):
    """Tests complets pour les fonctionnalités de base de données externe"""
    ...
    def test_external_db_operations(self):
        ...
        self.db_manager.delete_article("REF001")
        self.db_manager.delete_article("REF002")
        ...
        result = self.external_connector.import_from_external_db(self.db_manager)
        self.assertTrue(result)
