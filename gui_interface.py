...
from stock_functions import StockManager # Sera adapté ou moins utilisé directement par l'UI
...
class LoginDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ttk.Label(master, text="Nom d'utilisateur:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        self.username_entry = ttk.Entry(master, width=25)
        self.username_entry.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(master, text="Code PIN:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
...
class ModernStockManagerApp:
    ...
    def __init__(self, root):
        self.root = root
        self.db_manager = DatabaseManager() # Utilise le chemin par défaut "data/stock_database.db"
        self.db_manager.connect()
        self.db_manager.create_tables() # S'assure que toutes les tables, y compris utilisateurs, sont créées
        self.db_manager.migrate_old_database() # Applique les migrations si nécessaire

        self.auth_manager = AuthManager(self.db_manager)
        self.stock_logic = StockManager(self.db_manager, self.auth_manager) # StockManager aura besoin de l'auth_manager pour l'id_utilisateur
    ...
    def perform_login(self):
        ...
        if not self.db_manager.get_user_by_username("admin"):
            ...
            messagebox.showinfo("Configuration initiale", "Un utilisateur 'admin' avec le PIN 'admin123' a été créé. Veuillez le changer.")
        ...
        while login_attempt < max_attempts:
            ...
            if dialog.result:
                ...
                if self.auth_manager.login(username, pin):
                    ...
                    messagebox.showerror("Échec de la connexion", f"Nom d'utilisateur ou PIN incorrect. Tentatives restantes: {max_attempts - login_attempt - 1}")
                    login_attempt += 1
            else: # L'utilisateur a fermé la boîte de dialogue
                return False
        messagebox.showerror("Échec de la connexion", "Nombre maximal de tentatives atteint.")
        return False

    def setup_main_window(self):
        """Configure la fenêtre principale de l'application après une connexion réussie."""
        self.root.title(f"Gestionnaire de Stock Pro - Swisspro (Utilisateur: {self.auth_manager.get_current_user()['username']})")
    ...
    def setup_style(self):
        ...
        self.style.theme_use('clam') # Thème de base plus moderne que 'default'

        self.style.configure("TFrame", background=self.colors['background'])
        self.style.configure("TLabel", background=self.colors['background'], foreground=self.colors['text'], font=("Roboto", 10))
        self.style.configure("Header.TLabel", font=("Roboto Medium", 16), foreground=self.colors['primary'])
        ...
        self.style.configure("TButton", background=self.colors['secondary'], foreground='white', font=button_font, borderwidth=0, padding=(10,5))
        ...
        for style_name, color_key in [
            ...
            ("Info.TButton", 'info')]:
            self.style.configure(style_name, background=self.colors[color_key], foreground='white')
            self.style.map(style_name, background=[('active', self.shade_color(self.colors[color_key], -0.2)), ('pressed', self.shade_color(self.colors[color_key], -0.2))])

        self.style.configure("TEntry", foreground=self.colors['text'], fieldbackground='white', font=("Roboto", 10), padding=5)
        self.style.configure("TCombobox", foreground=self.colors['text'], fieldbackground='white', font=("Roboto", 10), padding=5)
        
        self.style.configure("Treeview", background='white', foreground=self.colors['text'], rowheight=30, fieldbackground='white', font=("Roboto", 10))
        self.style.map("Treeview", background=[('selected', self.colors['secondary'])], foreground=[('selected', 'white')])
        self.style.configure("Treeview.Heading", background=self.colors['primary'], foreground='white', font=("Roboto Medium", 10), padding=5, relief='flat')
        self.style.map("Treeview.Heading", background=[('active', self.colors['secondary'])])
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Importer depuis Excel...", command=self.import_excel_secure)
        file_menu.add_command(label="Exporter tout (Excel)...", command=lambda: self.export_data_to_excel(all_data=True))
        file_menu.add_command(label="Exporter sélection (Excel)...", command=lambda: self.export_data_to_excel(all_data=False))
        file_menu.add_command(label="Exporter articles à commander (Excel)...", command=self.export_low_stock_to_excel)
        file_menu.add_separator()
        file_menu.add_command(label="Changer d'utilisateur", command=self.logout_and_relogin)
        file_menu.add_command(label="Quitter", command=self.on_closing)
        menubar.add_cascade(label="Fichier", menu=file_menu)

        # Menu Stock
        stock_menu = Menu(menubar, tearoff=0)
        stock_menu.add_command(label="Nouvel Article...", command=self.add_new_article_dialog, style="Success.TButton")
        stock_menu.add_command(label="Modifier Article Sélectionné...", command=self.edit_selected_article_dialog)
        stock_menu.add_command(label="Supprimer Article Sélectionné", command=self.delete_selected_article, style="Danger.TButton")
        stock_menu.add_separator()
        stock_menu.add_command(label="Ajouter Stock...", command=lambda: self.add_or_remove_stock_dialog("AJOUT"))
        stock_menu.add_command(label="Retirer Stock...", command=lambda: self.add_or_remove_stock_dialog("RETRAIT"))
        stock_menu.add_command(label="Historique de l'article...", command=self.show_article_history_dialog)
        ...
        tools_menu.add_command(label="Sauvegardes...", command=self.show_backup_window)
        menubar.add_cascade(label="Outils", menu=tools_menu)
        
        # Menu Administration (visible seulement pour les admins)
        if self.auth_manager.is_admin():
            admin_menu = Menu(menubar, tearoff=0)
            admin_menu.add_command(label="Gérer les utilisateurs...", command=self.show_user_management_window)
    ...
    def setup_ui(self):
        ...
        details_actions_panel.pack_propagate(False) # Empêche le redimensionnement par les enfants
        self.create_article_details_form(details_actions_panel) # Remplacé
        self.create_quick_actions(details_actions_panel) # Nouveaux boutons d'action rapide
        
        # Section Inférieure: Barre d'état et Notifications
        bottom_frame = ttk.Frame(self.root, padding=(10,5), relief='sunken', borderwidth=1)
    ...
    def create_header(self, parent):
        ...
        try:
            logo_path = os.path.join("data", "logo.jpg") # Assurez-vous que ce chemin est correct
            if os.path.exists(logo_path):
                logo_image = Image.open(logo_path).resize((120, 40), Image.LANCZOS)
                self.logo_photo = ImageTk.PhotoImage(logo_image)
                ttk.Label(header_content_frame, image=self.logo_photo, background=self.colors['background']).pack(side=tk.LEFT, padx=(0,10))
        except Exception as e:
            print(f"Erreur chargement logo: {e}")
        
        ttk.Label(header_content_frame, text="GESTIONNAIRE DE STOCK PRO", style="Header.TLabel").pack(side=tk.LEFT)
        self.current_user_label = ttk.Label(header_content_frame, text=f"Utilisateur: {self.auth_manager.get_current_user()['username']} ({self.auth_manager.get_current_user()['role']})", style="Status.TLabel")
    ...
    def create_search_bar(self, parent):
        ...
        ttk.Button(search_frame, text="Chercher", command=self.search_articles, style="Info.TButton").pack(side=tk.LEFT, padx=5)
        ttk.Button(search_frame, text="Réinitialiser", command=self.reset_search).pack(side=tk.LEFT)

    def create_article_list(self, parent):
        list_frame_container = ttk.LabelFrame(parent, text="Inventaire des Articles", padding=10)
        list_frame_container.pack(fill=tk.BOTH, expand=True)

        columns = ("reference", "description", "quantite", "quantite_minimale", "position")
        self.article_tree = ttk.Treeview(list_frame_container, columns=columns, show="headings", selectmode="browse")
        ...
        for col_id, (col_text, col_width) in col_map.items():
            self.article_tree.heading(col_id, text=col_text, command=lambda c=col_id: self.sort_treeview(c, False))
            self.article_tree.column(col_id, width=col_width, anchor=tk.W if col_id == "description" else tk.CENTER)

        # Tags pour le style des lignes (stock bas)
        self.article_tree.tag_configure('low_stock', foreground=self.colors['danger'], font=('Roboto Medium', 10))
        self.article_tree.tag_configure('normal_stock', foreground=self.colors['text'])

        scrollbar_y = ttk.Scrollbar(list_frame_container, orient=tk.VERTICAL, command=self.article_tree.yview)
        scrollbar_x = ttk.Scrollbar(list_frame_container, orient=tk.HORIZONTAL, command=self.article_tree.xview)
        self.article_tree.configure(yscrollcommand=scrollbar_y.set, xscrollcommand=scrollbar_x.set)
    ...
    def create_article_details_form(self, parent):
        details_frame = ttk.LabelFrame(parent, text="Détails de l'Article Sélectionné", padding=15)
        details_frame.pack(fill=tk.X, pady=(0,10))

        self.detail_vars = {}
        fields = [
            ("Référence:", "reference", False), ("Description:", "description", True),
            ("Quantité Actuelle:", "quantite", False), ("Quantité Minimale:", "quantite_minimale", True),
        ...
        for i, (label_text, key, editable) in enumerate(fields):
            ttk.Label(details_frame, text=label_text).grid(row=i, column=0, sticky=tk.W, pady=3)
            var = tk.StringVar()
            ...
            if not editable:
                 detail_widget = ttk.Label(details_frame, textvariable=var, anchor=tk.W, relief='groove', padding=2)
            else:
                 detail_widget = ttk.Entry(details_frame, textvariable=var, state=entry_state, width=25)
            detail_widget.grid(row=i, column=1, sticky=tk.EW, pady=3, padx=5)
        
        details_frame.columnconfigure(1, weight=1) # Permet à la colonne des entrées de s'étendre

    def create_quick_actions(self, parent):
        actions_frame = ttk.LabelFrame(parent, text="Actions Rapides", padding=15)
        actions_frame.pack(fill=tk.X, pady=10)

        ttk.Button(actions_frame, text="Ajouter Stock", command=lambda: self.add_or_remove_stock_dialog("AJOUT"), style="Success.TButton").pack(fill=tk.X, pady=3)
        ttk.Button(actions_frame, text="Retirer Stock", command=lambda: self.add_or_remove_stock_dialog("RETRAIT"), style="Warning.TButton").pack(fill=tk.X, pady=3)
        ttk.Button(actions_frame, text="Modifier Article", command=self.edit_selected_article_dialog).pack(fill=tk.X, pady=3)
        ttk.Button(actions_frame, text="Historique Article", command=self.show_article_history_dialog, style="Info.TButton").pack(fill=tk.X, pady=3)

    def create_status_bar(self, parent):
        self.status_var = tk.StringVar()
        self.status_var.set("Prêt.")
        status_label = ttk.Label(parent, textvariable=self.status_var, style="Status.TLabel", anchor=tk.W)
        status_label.pack(side=tk.LEFT, fill=tk.X, expand=True)

    def create_notifications_area(self, parent):
        self.notifications_area = ttk.Label(parent, text="", style="Accent.TLabel", anchor=tk.E)
    ...
    def load_articles(self, sort_by=None, sort_desc=False):
        ...
        articles_data = self.stock_logic.get_all_articles() # Devrait retourner une liste de dictionnaires

        if sort_by:
            articles_data.sort(key=lambda x: x.get(sort_by, ''), reverse=sort_desc)

        for article in articles_data:
            values = (
                article.get('reference', ''), article.get('description', ''), 
                article.get('quantite', 0), article.get('quantite_minimale', 0),
                article.get('position', '')
            )
            tag = 'low_stock' if article.get('quantite', 0) < article.get('quantite_minimale', 0) else 'normal_stock'
    ...
    def sort_treeview(self, col, descending):
        data = [(self.article_tree.set(child, col), child) for child in self.article_tree.get_children('')]
        ...
        try:
            ...
            data.sort(key=lambda t: str(t[0]).lower(), reverse=descending) # Tri alphabétique sinon

        for index, (val, child) in enumerate(data):
            self.article_tree.move(child, '', index)
        self.article_tree.heading(col, command=lambda: self.sort_treeview(col, not descending))
    ...
    def on_article_select(self, event=None):
        ...
        self.selected_article_reference = self.article_tree.item(selected_item)['values'][0]
        
        article_data = self.stock_logic.get_article_by_reference(self.selected_article_reference)
        if article_data:
            for key, var in self.detail_vars.items():
                value = article_data.get(key, "")
                if isinstance(value, datetime) or ("date" in key and value):
                     try: var.set(pd.to_datetime(value).strftime('%d/%m/%Y %H:%M'))
                     except: var.set(str(value))
                else:
                    var.set(str(value) if value is not None else "")
            self.update_status(f"Article sélectionné: {article_data['reference']}")
        else:
            self.selected_article_reference = None # Réinitialiser si l'article n'est pas trouvé
            for key in self.detail_vars:
                self.detail_vars[key].set("")
        ...
        try:
            ...
            if articles_to_notify:
                ...
                for art in articles_to_notify[:3]: # Afficher les 3 premiers
                    notif_details.append(f"{art['reference']} (Qté: {art['quantite_actuelle']})" )
                self.notifications_area.config(text=notif_text + ", ".join(notif_details) + ("..." if len(articles_to_notify) > 3 else ""))
    ...
    def add_new_article_dialog(self):
        ...
        dialog = ArticleDialog(self.root, title="Nouvel Article", stock_logic=self.stock_logic, auth_manager=self.auth_manager)
        if dialog.result:
    ...
    def edit_selected_article_dialog(self):
        if not self.selected_article_reference:
            messagebox.showwarning("Aucune sélection", "Veuillez sélectionner un article à modifier.")
            return
        article_data = self.stock_logic.get_article_by_reference(self.selected_article_reference)
        if not article_data:
            messagebox.showerror("Erreur", "Impossible de charger les données de l'article sélectionné.")
            return
        
        dialog = ArticleDialog(self.root, title="Modifier Article", article_data=article_data, stock_logic=self.stock_logic, auth_manager=self.auth_manager)
        if dialog.result:
            self.refresh_data_and_ui()
    ...
    def delete_selected_article(self):
        if not self.selected_article_reference:
            messagebox.showwarning("Aucune sélection", "Veuillez sélectionner un article à supprimer.")
            return
        if messagebox.askyesno("Confirmation", f"Êtes-vous sûr de vouloir supprimer l'article {self.selected_article_reference}?"):
            if self.stock_logic.delete_article(self.selected_article_reference):
                ...
                messagebox.showerror("Erreur", "La suppression de l'article a échoué.")
    ...
    def add_or_remove_stock_dialog(self, mode="AJOUT"):
        ...
        dialog = StockMovementDialog(self.root, title=title, reference=self.selected_article_reference, mode=mode, stock_logic=self.stock_logic, auth_manager=self.auth_manager)
        if dialog.result:
            self.refresh_data_and_ui()
            messagebox.showinfo("Succès", f"Stock mis à jour pour {self.selected_article_reference}.")

    def show_article_history_dialog(self):
        if not self.selected_article_reference:
            messagebox.showwarning("Aucune sélection", "Veuillez sélectionner un article pour voir son historique.")
            return
        HistoryDialog(self.root, self.selected_article_reference, self.stock_logic)
    ...
    def export_data_to_excel(self, all_data=True):
        ...
        if all_data:
            ...
            article = self.stock_logic.get_article_by_reference(self.selected_article_reference)
            if article: articles_to_export.append(article)
        else:
            messagebox.showwarning("Exportation de la sélection", "Aucun article sélectionné pour l'exportation.")
            return
        ...
        try:
            ...
            cols_to_export = ['reference', 'description', 'quantite', 'quantite_minimale', 'position', 'date_creation', 'date_modification']
            df = df[[col for col in cols_to_export if col in df.columns]]
            df.to_excel(file_path, index=False)
            messagebox.showinfo("Exportation Réussie", f"Données exportées avec succès vers {file_path}")
        except Exception as e:
    ...
    def export_low_stock_to_excel(self):
        ...
        if not low_stock_articles:
            messagebox.showinfo("Exportation Stock Bas", "Aucun article en stock bas à exporter.")
            return
        ...
        try:
            ...
            cols_to_export = ['reference', 'description', 'quantite', 'quantite_minimale', 'position']
            df = df[[col for col in cols_to_export if col in df.columns]]
            df['a_commander'] = df['quantite_minimale'] - df['quantite'] # Calcul de la quantité à commander
            df.to_excel(file_path, index=False)
            messagebox.showinfo("Exportation Réussie", f"Articles à commander exportés vers {file_path}")
        except Exception as e:
            messagebox.showerror("Erreur d'exportation", f"Une erreur est survenue: {e}")
...
class ArticleDialog(simpledialog.Dialog):
    def __init__(self, parent, title, stock_logic, auth_manager, article_data=None):
        ...
        self.article_data = article_data # None pour un nouvel article, données existantes pour modification
        self.fields = {}
        super().__init__(parent, title)

    def body(self, master):
        labels = ["Référence:", "Description:", "Quantité Initiale:", "Quantité Minimale:", "Position:"]
        keys = ["reference", "description", "quantite", "quantite_minimale", "position"]
        ...
        for i, (text, key) in enumerate(zip(labels, keys)):
            ttk.Label(master, text=text).grid(row=i, column=0, sticky=tk.W, padx=5, pady=2)
    ...
    def validate(self):
        ...
        if not values["reference"]:
            messagebox.showerror("Erreur de validation", "La référence est obligatoire.", parent=self)
            return False
        if not values["description"]:
            messagebox.showerror("Erreur de validation", "La description est obligatoire.", parent=self)
            return False
        try:
            if not self.article_data or self.fields["quantite"].cget("state") == tk.NORMAL:
                 qty = int(values.get("quantite", 0) or 0)
                 if qty < 0: raise ValueError("La quantité ne peut pas être négative.")
            min_qty = int(values.get("quantite_minimale", 0) or 0)
            if min_qty < 0: raise ValueError("La quantité minimale ne peut pas être négative.")
        except ValueError as e:
            messagebox.showerror("Erreur de validation", f"Valeur numérique invalide: {e}", parent=self)
            return False
        return True
    ...
    def apply(self):
        ...
        if self.article_data: # Mode modification
            ...
            qty = int(values.get("quantite", 0) or 0)
            success, msg_or_code = self.stock_logic.add_article(ref, desc, qty, pos, min_qty)
            if not success and msg_or_code == "EXISTS":
                ...
                messagebox.showerror("Erreur", f"La référence '{ref}' existe déjà.", parent=self)
                self.result = False
                return
        
        self.result = success
        if not success:
             messagebox.showerror("Erreur", "L'opération sur l'article a échoué.", parent=self)
...
class StockMovementDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ttk.Label(master, text=f"Article: {self.reference}").grid(row=0, column=0, columnspan=2, sticky=tk.W, padx=5, pady=5)
        
        ttk.Label(master, text="Quantité:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
        self.fields["quantite"] = ttk.Entry(master, width=15)
        self.fields["quantite"].grid(row=1, column=1, sticky=tk.EW, padx=5, pady=2)
        
        ttk.Label(master, text="Projet (optionnel):").grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
        self.fields["projet"] = ttk.Entry(master, width=30)
        self.fields["projet"].grid(row=2, column=1, sticky=tk.EW, padx=5, pady=2)
        ...
        ttk.Label(master, text="Travailleur:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=2)
        self.fields["travailleur"] = ttk.Entry(master, width=30)
        self.fields["travailleur"].insert(0, current_username)
        self.fields["travailleur"].grid(row=3, column=1, sticky=tk.EW, padx=5, pady=2)
    ...
    def validate(self):
        try:
            ...
            if quantite <= 0:
                messagebox.showerror("Validation", "La quantité doit être un nombre positif.", parent=self)
                return False
        except ValueError:
            messagebox.showerror("Validation", "La quantité doit être un nombre valide.", parent=self)
            return False
        return True
    ...
    def apply(self):
        ...
        if self.mode == "AJOUT":
            success = self.stock_logic.add_stock(self.reference, quantite, projet, travailleur)
        else: # RETRAIT
            success = self.stock_logic.remove_stock(self.reference, quantite, projet, travailleur)
        
        self.result = success
        if not success:
            messagebox.showerror("Erreur", f"L'opération de {self.mode.lower()} de stock a échoué.", parent=self)
...
class HistoryDialog(tk.Toplevel):
    ...
    def __init__(self, parent, reference, stock_logic):
        ...
        movements = stock_logic.get_article_movements(reference) # Attendre une liste de dictionnaires

        cols = ["date_mouvement", "type_mouvement", "qte_avant", "qte_changee", "qte_apres", "projet", "travailleur", "nom_utilisateur"]
        ...
        if movements:
            for mov in movements:
                ...
                values = (
                    pd.to_datetime(mov.get('date_mouvement')).strftime('%d/%m/%Y %H:%M') if mov.get('date_mouvement') else '',
                    mov.get('type_mouvement', ''),
                    mov.get('quantite_avant_mouvement', ''),
                    mov.get('quantite_change', ''),
                    mov.get('quantite_apres_mouvement', ''),
                    mov.get('projet', ''),
                    mov.get('travailleur', ''), # Ce champ est peut-être redondant avec nom_utilisateur
                    mov.get('nom_utilisateur', '')
                )
                tree.insert("", tk.END, values=values)
        else:
            tree.insert("", tk.END, values=("Aucun mouvement trouvé", "", "", "", "", "", "", ""))
...
class StatisticsWindow(tk.Toplevel):
    ...
    def __init__(self, parent, stats_manager):
        ...
        ttk.Label(main_frame, text="Statistiques Générales", font=("Roboto Medium", 14)).pack(pady=10)
        ...
        summary_text_area = tk.Text(main_frame, height=15, width=80, font=("Roboto", 10), relief='sunken', borderwidth=1)
        summary_text_area.pack(pady=10, fill=tk.BOTH, expand=True)
        summary_text_area.insert(tk.END, f"Statistiques Sommaires:\n-----------------------\n")
        summary_text_area.insert(tk.END, f"Total Mouvements: {summary['total_mouvements']}\n")
        summary_text_area.insert(tk.END, f"Total Quantité Entrée: {summary['total_quantite_entree']}\n")
        summary_text_area.insert(tk.END, f"Total Quantité Sortie: {summary['total_quantite_sortie']}\n\n")
        
        summary_text_area.insert(tk.END, "Articles les plus mouvementés:\n")
        for item in summary['articles_plus_mouvementes']:
            summary_text_area.insert(tk.END, f"  - {item['article']}: {item['nombre_mouvements']} mouvements\n")
        summary_text_area.insert(tk.END, "\nUtilisateurs les plus actifs:\n")
        for item in summary['utilisateurs_plus_actifs']:
            summary_text_area.insert(tk.END, f"  - {item['utilisateur']}: {item['nombre_actions']} actions\n")
...
class BackupWindow(tk.Toplevel):
    ...
    def __init__(self, parent, backup_manager):
        ...
        ttk.Button(main_frame, text="Créer une Sauvegarde Manuelle Maintenant", command=self.create_manual_backup, style="Primary.TButton").pack(pady=10, fill=tk.X)

        list_frame = ttk.LabelFrame(main_frame, text="Sauvegardes Disponibles", padding=10)
        list_frame.pack(pady=10, fill=tk.BOTH, expand=True)

        self.backup_listbox = tk.Listbox(list_frame, height=10, font=("Roboto", 10))
        self.backup_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.backup_listbox.yview)
        ...
        ttk.Button(action_frame, text="Restaurer la Sélection", command=self.restore_selected_backup, style="Warning.TButton").pack(side=tk.LEFT, padx=5, expand=True)
        ttk.Button(action_frame, text="Supprimer Anciennes (max 7)", command=self.manage_old_backups).pack(side=tk.LEFT, padx=5, expand=True)
    ...
    def create_manual_backup(self):
        ...
        if backup_path:
            messagebox.showinfo("Sauvegarde", f"Sauvegarde manuelle créée: {os.path.basename(backup_path)}", parent=self)
            self.refresh_backup_list()
        else:
            messagebox.showerror("Sauvegarde", "Échec de la création de la sauvegarde manuelle.", parent=self)

    def restore_selected_backup(self):
        selected_indices = self.backup_listbox.curselection()
        if not selected_indices:
            messagebox.showwarning("Restauration", "Veuillez sélectionner une sauvegarde à restaurer.", parent=self)
            return
        ...
        if messagebox.askyesno("Confirmation", 
            ...
            if self.backup_manager.restore_backup(selected_backup_name):
                messagebox.showinfo("Restauration", "Base de données restaurée avec succès. Veuillez redémarrer l'application.", parent=self)
                ...
                messagebox.showerror("Restauration", "Échec de la restauration de la sauvegarde.", parent=self)

    def manage_old_backups(self):
        # Par défaut, conserve 7 sauvegardes "normales"
        self.backup_manager.manage_backups(max_backups=7)
        messagebox.showinfo("Gestion des Sauvegardes", "Les anciennes sauvegardes ont été gérées.", parent=self)
        self.refresh_backup_list()
...
class UserManagementWindow(tk.Toplevel):
    ...
    def __init__(self, parent, db_manager, auth_manager):
        ...
        user_list_frame = ttk.LabelFrame(main_frame, text="Utilisateurs Existants", padding=10)
        ...
        user_scrollbar = ttk.Scrollbar(user_list_frame, orient=tk.VERTICAL, command=self.user_tree.yview)
        ...
        ttk.Button(action_frame, text="Ajouter Utilisateur", command=self.add_user_dialog, style="Success.TButton").pack(side=tk.LEFT, padx=5)
        # ttk.Button(action_frame, text="Modifier PIN/Rôle", command=self.edit_user_dialog).pack(side=tk.LEFT, padx=5) # Plus complexe
        ttk.Button(action_frame, text="Supprimer Utilisateur", command=self.delete_user_dialog, style="Danger.TButton").pack(side=tk.LEFT, padx=5)

        ttk.Button(main_frame, text="Fermer", command=self.destroy).pack(pady=10, side=tk.BOTTOM)

    def load_users(self):
        for i in self.user_tree.get_children():
            self.user_tree.delete(i)
        try:
            self.db_manager.cursor.execute("SELECT id, nom_utilisateur, role FROM utilisateurs ORDER BY nom_utilisateur")
            users = self.db_manager.cursor.fetchall()
            for user in users:
                self.user_tree.insert("", tk.END, values=user)
        except Exception as e:
            messagebox.showerror("Erreur", f"Impossible de charger les utilisateurs: {e}", parent=self)
    ...
    def delete_user_dialog(self):
        ...
        if not selected_items:
            messagebox.showwarning("Sélection", "Veuillez sélectionner un utilisateur à supprimer.", parent=self)
            return
        user_id_to_delete = self.user_tree.item(selected_items[0])['values'][0]
        username_to_delete = self.user_tree.item(selected_items[0])['values'][1]
        ...
        if current_user and current_user['id'] == user_id_to_delete:
            messagebox.showerror("Interdit", "Vous ne pouvez pas supprimer l'utilisateur actuellement connecté.", parent=self)
            return
        if username_to_delete.lower() == 'admin' and current_user['username'].lower() != 'admin':
             messagebox.showerror("Interdit", "Seul l'utilisateur 'admin' principal peut supprimer d'autres admins (ou lui-même s'il n'est pas le dernier).", parent=self)
        ...
        if messagebox.askyesno("Confirmation", f"Supprimer l'utilisateur '{username_to_delete}' (ID: {user_id_to_delete})?", parent=self):
            try:
                self.db_manager.cursor.execute("DELETE FROM utilisateurs WHERE id = ?", (user_id_to_delete,))
                self.db_manager.conn.commit()
                self.load_users()
                messagebox.showinfo("Succès", "Utilisateur supprimé.", parent=self)
            except Exception as e:
                messagebox.showerror("Erreur", f"Impossible de supprimer l'utilisateur: {e}", parent=self)
...
class AddUserDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ttk.Label(master, text="Nom d'utilisateur:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=2)
        self.fields['username'] = ttk.Entry(master, width=25)
        self.fields['username'].grid(row=0, column=1, padx=5, pady=2)

        ttk.Label(master, text="PIN (min. 4 chiffres):").grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
        self.fields['pin'] = ttk.Entry(master, show="*", width=25)
        self.fields['pin'].grid(row=1, column=1, padx=5, pady=2)

        ttk.Label(master, text="Confirmer PIN:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
        self.fields['pin_confirm'] = ttk.Entry(master, show="*", width=25)
        self.fields['pin_confirm'].grid(row=2, column=1, padx=5, pady=2)

        ttk.Label(master, text="Rôle:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=2)
        self.fields['role'] = ttk.Combobox(master, values=["utilisateur", "admin"], state="readonly", width=23)
    ...
    def validate(self):
        ...
        if not username or not pin or not pin_confirm:
            messagebox.showerror("Validation", "Tous les champs sont obligatoires.", parent=self)
            return False
        if len(pin) < 4 or not pin.isdigit():
            messagebox.showerror("Validation", "Le PIN doit contenir au moins 4 chiffres.", parent=self)
            return False
        if pin != pin_confirm:
            messagebox.showerror("Validation", "Les PINs ne correspondent pas.", parent=self)
            return False
        return True
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    app = ModernStockManagerApp(root) # La logique de login est maintenant dans __init__
